...
export function WorkflowOutput() {
    const { streamingOutput, workflowStatus, messages, workflowConfig } =
        useWorkflowContext()
    ...
    const statusIcon = useMemo(() => {
        switch (workflowStatus) {
            ...
            case 'completed':
                return (
                    <CheckCircle2Icon className="size-3.5 text-green-500 glow-green" />
                )
            case 'error':
                return (
                    <AlertCircleIcon className="size-3.5 text-red-500 glow-red" />
                )
            case 'paused':
            case 'idle':
            default:
    ...
    return (
        <Panel
            ...
            className="w-96 p-0 bg-card/40 backdrop-blur-3xl border-border/20 shadow-2xl rounded-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300"
        >
                    ...
                    {streamingOutput ? (
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            {msg.role !== 'user' && (
                                                <SparklesIcon className="size-3 text-primary/40" />
                                            )}
                                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center h-40 text-center space-y-3">
                            ...
                            <p className="text-[11px] text-muted-foreground/60 italic px-8">
                                Initialize neural path to capture workflow
                                artifacts...
                            </p>
                        </div>
                    )}
