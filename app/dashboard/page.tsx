...
export default function DashboardPage() {
    ...
    const { data: traces, isLoading: tracesLoading } = useTracesQuery({
        perPage: 5,
    })
                                ...
                                <div className="divide-y">
                                    {agents.slice(0, 5).map((agent) => (
                                        <Link
                                            key={agent.id}
                                            href={
                                                `/dashboard/agents?agent=${agent.id}` as Route
                                            }
                                ...
                                <div className="divide-y">
                                    {workflows.slice(0, 5).map((wf) => (
                                        <Link
                                            key={wf.id}
                                            href={
                                                `/dashboard/workflows?workflow=${wf.id}` as Route
                                            }
                                            className="flex items-center justify-between p-4 hover:bg-accent/50 transition-colors"
                                        >
                                            <div className="flex items-center gap-3">
                            ...
                            <div className="divide-y">
                                {traces.spans.slice(0, 5).map((span) => (
                                    <Link
                                        key={span.traceId}
                                        href={
                                            `/dashboard/observability?trace=${span.traceId}` as Route
                                        }
                                        className="flex items-center justify-between p-4 hover:bg-accent/50 transition-colors"
                                    >
                                        <div className="flex items-center gap-3">
        ...
        <Link href={href as Route}>
            <Card className="hover:bg-accent/50 hover:border-accent transition-all cursor-pointer h-full group">
                <CardHeader className="pb-3">
                    <div className="flex items-center gap-3">
                        <div
                            className={`flex h-10 w-10 items-center justify-center rounded-lg ${colorVariants[color]} transition-transform group-hover:scale-110`}
                        >
