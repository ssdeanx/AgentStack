...
export default function MemoryPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useMemoryThreads(resourceId, agentId)
  ...
  const threadItems: MemoryThreadItem[] = Array.isArray(threads?.threads)
    ? threads.threads
      .map((thread) => {
        const rec = thread as Record<string, unknown>
      ...
      .filter(thread => thread !== null) as MemoryThreadItem[]
    : []

  const filteredThreads = threadItems.filter((thread) => {
    const q = searchQuery.toLowerCase()
    return (
      (thread.title?.toLowerCase().includes(q) ?? false) ||
  ...
  const handleCreateThread = async () => {
    try {
      ...
      if (typeof (thread as { id?: unknown }).id === 'string') {
        setSelectedThreadId((thread as { id: string }).id)
      }
    } catch (err) {
            ...
            <div className="space-y-1 p-2">
              {filteredThreads.map((thread) => (
                <button
                  ...
                  className={`w-full rounded-md p-3 text-left transition-colors hover:bg-accent ${selectedThreadId === thread.id
                      ? 'bg-accent'
                      : ''
                    }`}
                >
...
function ThreadDetails({
  ...
  const { messages, loading, error, refetch } = useMemoryThread(
    threadId,
    agentId
  )
  const { data: workingMemory, loading: wmLoading } = useWorkingMemory(
    agentId,
    threadId,
    resourceId
  )
  ...
  const workingMemoryView: WorkingMemoryView =
    workingMemory !== null && typeof workingMemory === 'object'
      ? {
        workingMemory:
          typeof (
            workingMemory as Record<string, unknown>
          ).workingMemory === 'string'
            ? ((workingMemory as Record<string, unknown>)
              .workingMemory as string)
            : undefined,
        source:
          typeof (workingMemory as Record<string, unknown>).source ===
            'string'
            ? ((workingMemory as Record<string, unknown>)
              .source as string)
            : undefined,
                ...
                <div className="space-y-4">
                  {messages.length > 0 ? (
                    messages.map((message, index) => {
                      const content = (() => {
                        const mc = message.content as
                          | { content: string }
                          | { parts: { text?: string; type?: string }[] }

                        if ('content' in mc && typeof mc.content === 'string') {
                          return mc.content
                        }

                        if ('parts' in mc && Array.isArray(mc.parts)) {
                          return mc.parts
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            className={`rounded-lg p-3 max-w-[80%] ${message.role ===
                                'user'
                ...
                >
                  <DialogTrigger asChild>
                    <Button
                      ...
                      onClick={() =>
                        setWmContent(
                          workingMemoryView.workingMemory ??
                          ''
                        )
                      }
                    >
                ...
                <div className="space-y-4">
                  <div className="flex gap-2">
                    <Badge variant="secondary">
                      Source:{' '}
                      {workingMemoryView.source}
                    </Badge>
                    {workingMemoryView.threadExists && (
