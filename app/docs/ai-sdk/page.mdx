...
export const mastra = new Mastra({
    agents: { weatherAgent },
    server: {
        apiRoutes: [chatRoute({ path: '/chat', agent: 'weather-agent' })],
    },
})
```
...
export const mastra = new Mastra({
    server: {
        apiRoutes: [networkRoute({ path: '/network', agent: 'agentNetwork' })],
    },
})
```
...
export const AgentTool = ({ id, text, status }) => (
    <Tool>
        <ToolHeader
            type={`${id}`}
            state={
                status === 'finished' ? 'output-available' : 'input-available'
            }
        />
        <ToolContent>
