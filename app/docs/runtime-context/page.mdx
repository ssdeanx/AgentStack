...
> Important: Only copy _trusted_ or validated fields into `runtimeContext` from client input. Sensitive keys should be added on the server side only.
...
memory: ({ runtimeContext }) => {
    ...
    if (userTier === 'enterprise') {
        return new Memory({
            storage: enterpriseStore,
            options: { semanticRecall: { topK: 15 } },
        })
    }
    if (userTier === 'pro') {
        return new Memory({
            storage: proStore,
            options: { semanticRecall: { topK: 8 } },
        })
    }
    return new Memory({
        storage: freeStore,
...
tools: ({ runtimeContext }) => {
    ...
    if (userTier === 'pro' || userTier === 'enterprise')
        allowed.push(advancedAnalytics)
    return allowed
}
```
