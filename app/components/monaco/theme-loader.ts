...
const themeLoaders = {
    ...
    monokai: () =>
        import('./themes/Monokai.json', { assert: { type: 'json' } }),
    dracula: () =>
        import('./themes/Dracula.json', { assert: { type: 'json' } }),
    solarizedDark: () =>
        import('./themes/Solarized-dark.json', { assert: { type: 'json' } }),
...
export async function applyTheme(themeId: string) {
    const safeTheme = (
        availableThemes.some((t) => t.id === themeId) ? themeId : 'github'
