...
<BlogLayout title="Hello World with AgentStack (Mastra): A 10-minute Guide" date="2025-12-01" readTime="10 min read" category="Tutorial" author="AgentStack Team" slug="hello-world-agentstack">
...
export const helloWorldAgent = new Agent({
    ...
    instructions: () => ({
        role: 'system',
        content:
            'You are a very friendly agent who responds with a short greeting.',
    }),
...
export const pgStore = new PostgresStore({
    connectionString:
        process.env.SUPABASE ?? 'postgresql://user:password@localhost/mydb',
