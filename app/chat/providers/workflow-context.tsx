...
function buildWorkflowInputData(
    workflowId: string,
    inputText: string
...
export function WorkflowProvider({
    ...
    const [selectedWorkflow, setSelectedWorkflow] =
        useState<WorkflowId>(defaultWorkflow)
    ...
    >([])
    const [suspendPayload, setSuspendPayload] =
        useState<WorkflowSuspendPayload | null>(null)
    ...
    const { messages, sendMessage, stop, status } = useChat({
        transport: new DefaultChatTransport({
            ...
            prepareSendMessagesRequest({ messages: msgs }) {
                ...
                const metadata = (
                    last as unknown as { metadata?: Record<string, unknown> }
                )?.metadata
                const runId =
            ...
            if (message.role === 'assistant' && message.parts !== null) {
                for (const [partIndex, part] of message.parts.entries()) {
                    ...
                    if (part.type === 'data-tool-workflow') {
                        const nestedWorkflowPart = part as {
                            data?: {
                                text?: string
                                status?: string
                                workflowId?: string
                    ...
                    if (part.type === 'data-tool-network') {
                        const nestedNetworkPart = part as {
                            data?: {
                                text?: string
                                status?: string
                                networkId?: string
                    ...
                    if (part.type === 'data-workflow-suspend') {
                        ...
                        if (
                            suspendData?.message !== null &&
                            suspendData?.message !== undefined &&
                            suspendData?.stepId !== null &&
                            suspendData?.stepId !== undefined
                        ) {
                    ...
                    if (
                        typeof part.type === 'string' &&
                        part.type.startsWith('data-workflow-progress')
                    ) {
                    ...
                    if (
                        ...
                        const toolProgressPart = part as {
                            type: string
                            data?: {
                                status?: string
                                message?: string
    ...
    const runWorkflow = useCallback(
        (inputData?: Record<string, unknown>) => {
            ...
            void sendMessage({
                text: inputText,
                // AI SDK v6 supports message metadata; we use it to pass runId through prepareSendMessagesRequest.
    ...
    const resumeWorkflow = useCallback(
        (resumeData?: { approved: boolean; approverName?: string }) => {
    ...
    const runStep = useCallback(
        async (stepId: string) => {
            ...
            const stepIndex = workflowConfig.steps.findIndex(
                (s) => s.id === stepId
            )
            if (stepIndex === -1) {
                return
                    ...
                    ? {
                          ...
                          stepProgress: {
                              ...
                              [stepId]: {
                                  ...
                                  status: 'completed',
                                  startedAt:
                                      prev.stepProgress[stepId]?.startedAt,
