...
function getStatusIcon(status: StepStatus) {
    switch (status) {
        ...
        case 'running':
            return (
                <CircleDotIcon className="size-3 text-amber-500 animate-pulse" />
            )
        case 'error':
...
export const WorkflowNode = memo(({ data, selected }: WorkflowNodeProps) => {
    ...
    const stepStreamingEvents = progressEvents.filter(
        (event) => event.stepId === step.id && event.status === 'in-progress'
    )
            ...
            <div className="card-3d group">
                <Node
                    ...
                    className={cn(
                        ...
                        status === 'running' &&
                            'ring-2 ring-amber-500/50 glow-amber animate-ambient-pulse',
                    ...
                    <NodeHeader className="border-b border-border/10 bg-muted/20">
                        <div className="flex items-center justify-between w-full">
                            <div className="flex items-center gap-2">
                                ...
                                <NodeTitle className="text-sm font-bold tracking-tight uppercase opacity-80">
                                    {step.label}
                                </NodeTitle>
                            </div>
                            <Badge
                        ...
                        <NodeDescription className="text-[11px] leading-tight opacity-60 italic">
                            {step.description}
                        </NodeDescription>
                    </NodeHeader>
                            ...
                            stepStreamingEvents.length > 0 && (
                                <div className="mt-2 p-2 bg-amber-500/5 dark:bg-amber-400/5 rounded border border-amber-500/20 backdrop-blur-sm shadow-inner overflow-hidden relative">
                                    ...
                                    <div className="text-[11px] font-mono text-amber-700/90 dark:text-amber-300/90 max-h-24 overflow-y-auto custom-scrollbar">
                                        {stepStreamingEvents.map(
                                            (event, idx) => (
                                                <div
                                                    key={idx}
                                                    className="mb-1 last:mb-0 typing-effect"
                                                >
                                                    {event.message}
