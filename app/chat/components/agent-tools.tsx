...
export function AgentTools({ tools, className }: AgentToolsProps) {
    const groups = useMemo(() => {
        const grouped = new Map<
            string,
            Array<ToolInvocationState | DynamicToolUIPart>
        >()
        const order: string[] = []
    ...
    return (
        <div className={cn('space-y-2 mt-2', className)}>
            {groups.map(({ id, items }, groupIdx) => {
                ...
                const toolNameFromDynamic = (latest as { toolName?: unknown })
                    .toolName
                const toolName =
                    (typeof toolNameFromDynamic === 'string' &&
                    toolNameFromDynamic.trim().length > 0
                        ? toolNameFromDynamic
                        : undefined) ??
                    (typeof (latest as { type?: unknown }).type === 'string' &&
                    (latest as { type: string }).type.startsWith('tool-')
                        ? (latest as { type: string }).type.slice(
                              'tool-'.length
                          )
                        : 'unknown')
                ...
                const errorText =
                    toolState === 'output-error'
                        ? (latest as unknown as { errorText?: string })
                              .errorText
                        : undefined
                ...
                if (
                    ...
                    return (
                        <FinancialQuoteCard
                            ...
                            output={
                                latest.output as FinnhubQuotesUITool['output']
                            }
                            errorText={errorText}
                        />
                    )
                ...
                if (toolName === 'chart-supervisor' && hasOutput) {
                    return (
                        <FinancialChart
                            ...
                            toolCallId={id}
                            input={
                                latest.input as ChartSupervisorUITool['input']
                            }
                            output={
                                latest.output as ChartSupervisorUITool['output']
                            }
                            errorText={errorText}
                        />
                    )
                ...
                if (toolName === 'github-list-repositories' && hasOutput) {
                    return (
                        <RepositoryCard
                            ...
                            toolCallId={id}
                            input={
                                latest.input as ListRepositoriesUITool['input']
                            }
                            output={
                                latest.output as ListRepositoriesUITool['output']
                            }
                            errorText={errorText}
                        />
                    )
                ...
                if (toolName === 'github-list-pull-requests' && hasOutput) {
                    return (
                        <PullRequestList
                            ...
                            toolCallId={id}
                            input={
                                latest.input as ListPullRequestsUITool['input']
                            }
                            output={
                                latest.output as ListPullRequestsUITool['output']
                            }
                            errorText={errorText}
                        />
                    )
                ...
                if (
                    (toolName === 'finnhub-company' ||
                        toolName === 'polygon-stock-fundamentals') &&
                    hasOutput
                ) {
                    return (
                        <CompanyProfileCard
