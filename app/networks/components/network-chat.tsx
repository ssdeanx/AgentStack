...
} from '@/app/networks/providers/network-context'
import type {
    UIMessage,
    TextUIPart,
    ReasoningUIPart,
        ...
        <div className="space-y-2 mt-2">
            {tools.map((tool) => {
                ...
                const toolInput =
                    tool.type === 'dynamic-tool'
                        ? tool.input
                        : (tool as unknown as { args?: unknown }).args
                const toolOutput =
                    tool.type === 'dynamic-tool'
                        ? tool.output
                        : (tool as unknown as { result?: unknown }).result
                const toolErrorText =
                    tool.type === 'dynamic-tool'
                        ? tool.errorText
                        : (tool as unknown as { error?: unknown }).error
            ...
            {message.parts.map((part, i) => {
                ...
                switch (part.type) {
                    ...
                    case 'file': { // Handle file/image parts with AIImage
                        ...
                        if (
                            fileData.mediaType?.startsWith('image/') &&
                            Boolean(fileData.uint8Array)
                        ) {
                            const uint8Array =
                    ...
                    case 'data-tool-agent':
                        ...
                        return (
                            <AgentTool
                                key={key}
                                {...(part as { data: AgentDataPart }).data}
                            />
                        )
                    default:
                        ...
                        if (
                            ...
                            const toolPart =
                                part as unknown as DynamicToolUIPart
            ...
            <div className="perspective-distant preserve-3d py-2 reveal-on-scroll">
                <Message
                    ...
                    className={cn(
                        ...
                        isAssistant
                            ? 'rotate-y-2 translate-z-2 glass shadow-xl'
            ...
            {message.role === 'assistant' &&
                ...
                textPart && (
                    <MessageActions>
                        ...
                        <MessageAction label="Like" tooltip="Helpful response">
                            <ThumbsUpIcon className="size-3" />
                        </MessageAction>
                        <MessageAction label="Dislike" tooltip="Not helpful">
                        ...
                        <MessageAction label="View Code" tooltip="View as code">
...
export function NetworkChat() {
    ...
    const handleSubmit = (msg: PromptInputMessage) => {
        ...
        setTokenUsage((prev) => ({
            ...prev,
            input: prev.input + msg.text.length,
        }))
    }
            ...
            <Conversation className="flex-1">
                <ConversationContent>
                    ...
                    {pendingToolConfirmation && (
                        ...
                        >
                            <ConfirmationTitle>
                                Tool "{pendingToolConfirmation.toolName}" wants
                                to execute
                            </ConfirmationTitle>
                            <ConfirmationAccepted>
                            ...
                            <ConfirmationActions>
                                <ConfirmationAction
                                    variant="outline"
                                    onClick={pendingToolConfirmation.onReject}
                                >
                                    Reject
                                </ConfirmationAction>
                                <ConfirmationAction
                                    onClick={pendingToolConfirmation.onAccept}
                                >
                                    Approve
                                </ConfirmationAction>
                            </ConfirmationActions>
