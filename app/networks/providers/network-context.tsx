...
function mapDataPartToDynamicTool(part: MastraPart): DynamicToolUIPart | null {
    ...
    const rawState = (inner?.state ?? inner?.status ?? '')
        .toString()
        .toLowerCase()

    let state: DynamicToolUIPart['state'] = 'input-available'
    if (
        Boolean(rawState.includes('stream')) ||
        Boolean(rawState.includes('pending'))
    ) {
        state = 'input-streaming'
    } else if (
        Boolean(rawState.includes('success')) ||
        Boolean(rawState.includes('done')) ||
        Boolean(rawState.includes('completed')) ||
        Boolean(output)
    ) {
        state = 'output-available'
    } else if (
        Boolean(rawState.includes('error')) ||
        Boolean(rawState.includes('failed'))
    ) {
        state = 'output-error'
    }
...
export function NetworkProvider({
    ...
    const validDefaultNetwork =
        NETWORK_CONFIGS[defaultNetwork] !== undefined
            ? defaultNetwork
            : Object.keys(NETWORK_CONFIGS)[0]
    const [selectedNetwork, setSelectedNetwork] =
        useState<NetworkId>(validDefaultNetwork)
    ...
    const networkStatus: NetworkStatus = useMemo(() => {
        if (aiError || Boolean(networkError)) {
            return 'error'
        }
            ...
            if (message.role === 'assistant' && message.parts !== null) {
                for (const [partIndex, part] of message.parts.entries()) {
                    ...
                    if (
                        ...
                        const eventData = agentPart.data as
                            | Record<string, unknown>
                            | undefined
                        const dataObj = eventData?.data as
                            | Record<string, unknown>
                            | undefined
                        ...
                        const stage =
                            typeof eventData?.stage === 'string'
                                ? eventData.stage
                                : typeof dataObj?.stage === 'string'
                                  ? dataObj.stage
                                  : part.type.replace('data-tool-', '')
                        const agentId =
                    ...
                    if (
                        ...
                        part.type.startsWith('data-tool-progress')
                    ) {
                        const progressPart = part as {
                            type: string
                            data?: {
                                status?: string
                                message?: string
    ...
    const derivedRoutingSteps = useMemo(() => {
        ...
        const rs = normalizedRaw.toLowerCase()
            ...
            if (rs.includes('error') || rs.includes('failed')) {
                return 'error'
            }
            return 'pending'
        }
        ...
        const networkPart = dataParts.find((p) => p.type === 'data-network') as
            | NetworkDataPart
            | undefined
        if (networkPart) {
            const payload =
                ...
                (networkPart as NetworkPayload)
            const agentsFromPart =
                payload?.agents ?? payload?.nodes ?? payload?.steps ?? []
            ...
            if (Array.isArray(agentsFromPart) && agentsFromPart.length > 0) {
                return agentsFromPart.map((agent, idx) => {
                    ...
                    const agentName =
                        ...
                        String(agentId)
                    const input =
                        agent.input ?? agent.args ?? agent.params ?? ''
                    const output = agent.output ?? agent.result ?? agent.value
                    const status = mapRawStateToStatus(
        ...
        if (networkConfig?.agents?.length) {
            const steps: RoutingStep[] = networkConfig.agents.map(
                (agent, index) => ({
                    agentId: agent.id,
                    agentName: agent.name,
                    input: '',
                    status:
                        index === 0 && aiStatus === 'streaming'
                            ? 'active'
                            : 'pending',
                })
            )
        ...
        const toolParts = dataParts.filter(
            (p) =>
                p.type === 'dynamic-tool' ||
                (typeof p.type === 'string' && p.type.startsWith('data-tool-'))
        )
        if (toolParts.length > 0) {
            const steps: RoutingStep[] = toolParts.map((tp, idx) => {
                ...
                return {
                    ...
                    status: converted
                        ? converted.state === 'input-streaming'
                            ? 'active'
                            : converted.state === 'output-error'
                              ? 'error'
                              : converted.state === 'output-available'
                                ? 'completed'
                                : 'pending'
                        : 'pending',
                } as RoutingStep
