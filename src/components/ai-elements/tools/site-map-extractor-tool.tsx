...
import { ExternalLink, FileText, Link, TreePine } from 'lucide-react'
...
export function SiteMapExtractorTool({
  input,
  output,
  errorText,
}: SiteMapExtractorToolProps) {
  if (errorText) {
    return (
  ...
  const { pages, totalPages } = output as any

  // Group pages by depth for hierarchical display
  const pagesByDepth = pages.reduce(
    (acc: Record<number, typeof pages>, page: typeof pages[number]) => {
      if (!acc[page.depth]) {
        acc[page.depth] = []
      }
        ...
        <CardContent>
          <div className="flex items-center gap-4 text-sm">
            ...
            <Badge variant="secondary" className="text-purple-700">
              Depth: {Math.max(...pages.map((p: typeof pages[number]) => p.depth))}
            </Badge>
          </div>
        </CardContent>
      </Card>
                ...
                <div key={depth} className="space-y-2">
                  <div className="flex items-center gap-2 text-sm font-medium text-muted-foreground">
                    ...
                    <Badge
                      ...
                      className="text-xs"
                    >
                      {(depthPages as typeof pages).length} pages
                    </Badge>
                  </div>
