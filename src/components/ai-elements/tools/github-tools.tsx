...
>) {
  ...
  const repos = (output as { repositories?: Repository[] }).repositories ?? []
...
>) {
  ...
  const prs = (output as { pullRequests?: PullRequest[] }).pullRequests ?? []
...
export function IssueCard({
    ...
    return (
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-sm">
            <Loader2 className="size-4 animate-spin" />
            Fetching issue #{(input as any).issueNumber}...
          </CardTitle>
        </CardHeader>
      </Card>
    )
  ...
  const issue = (output as { issue?: Issue }).issue
