...
export function LinkExtractorTool({
  input,
  output,
  errorText,
}: LinkExtractorToolProps) {
  if (errorText) {
    return (
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-sm">
            ...
            Extracting links from {(input as { url: string }).url}...
          </CardTitle>
        </CardHeader>
        <CardContent>
  ...
  const { links, summary } = output as { links: any[], summary: any, url: string }
      ...
      <Card className="border-green-500/20 bg-green-500/5">
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-sm">
            ...
            Links Extracted from {(output as any).url}
          </CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {links.map((link: any, index: number) => (
              <div
                key={index}
                className={`flex items-center gap-3 p-3 rounded-lg border ${link.isValid
