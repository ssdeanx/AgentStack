...
const toRepoRelativePath = (value: string) => {
    const absolutePath = path.isAbsolute(value) ? value : path.resolve(mainFilesystem.basePath, value)
    return path.relative(mainFilesystem.basePath, absolutePath).split(path.sep).join('/')
}
...
const fileStep = createStep({
    ...
    execute: async ({ inputData, writer, requestContext, tracingContext, abortSignal }) => {
        ...
        const chosenPath = inputData.filePath ? toRepoRelativePath(inputData.filePath) : searchResults[0]?.path ?? searchResults[0]?.name
