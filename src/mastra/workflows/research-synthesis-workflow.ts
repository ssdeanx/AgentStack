...
const researchInputSchema = z.object({
    ...
    synthesisType: z
        .enum(['summary', 'comparative', 'comprehensive'])
        .default('summary'),
    maxSourcesPerTopic: z.number().default(5),
    concurrency: z
        .number()
        .default(2)
        .describe('Number of concurrent topic researches'),
})
...
const initializeResearchStep = createStep({
    ...
    execute: async ({ inputData, writer, mastra, requestContext }) => {
        ...
        logStepStart('initialize-research', {
    ...
    execute: async ({ inputData, mastra, writer, requestContext }) => {
        ...
        Focus on accuracy and citing credible sources.`
                ...
                result = {
                    ...
                    sources: [
                        {
                            title: `Source about ${inputData.topic}`,
                            relevance: 0.9,
                        },
                    ],
            ...
            logStepEnd(
                'research-topic-item',
                { topic: inputData.topic, confidence: result.confidence },
                Date.now() - startTime
            )
            return result
        } catch (error) {
...
const synthesizeResearchStep = createStep({
    ...
    description:
        'Synthesizes research across all topics using researchPaperAgent',
    inputSchema: z.object({
        topics: z.array(topicResearchSchema),
        synthesisType: z.string(),
    }),
    ...
    execute: async ({ inputData, mastra, writer, requestContext }) => {
        ...
        const startTime = Date.now()
        logStepStart('synthesize-research', {
            topicsCount: inputData.topics.length,
        })
        ...
        try {
            ...
            const agent =
                mastra?.getAgent('researchPaperAgent') ??
                mastra?.getAgent('researchAgent')

            let synthesis: z.infer<
                typeof synthesizedResearchSchema
            >['synthesis']
            ...
            if (agent !== undefined) {
                ...
                const topicsSummary = inputData.topics
                    .map(
                        (t) =>
...
5. Any gaps in the research`
            ...
            const totalSources = inputData.topics.reduce(
                (sum, t) => sum + t.sources.length,
                0
            )
            const averageConfidence =
                inputData.topics.reduce((sum, t) => sum + t.confidence, 0) /
                inputData.topics.length
...
const generateResearchReportStep = createStep({
    ...
    execute: async ({ inputData, mastra, writer, requestContext }) => {
        ...
        const startTime = Date.now()
        logStepStart('generate-research-report', {
...
export const researchSynthesisWorkflow = createWorkflow({
    ...
    description:
        'Multi-topic research with synthesis using .foreach() for concurrent topic processing',
    inputSchema: researchInputSchema,
    outputSchema: reportOutputSchema,
})
