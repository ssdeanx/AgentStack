    ...
    execute: async ({ inputData, writer, mastra, requestContext }) => {
        ...
        try {
            ...
            span?.update({
                output: result,
                metadata: {
                    'stock.currentPrice': result.currentPrice,
            ...
            logStepEnd(
                'fetch-stock-data',
                { symbol: result.symbol, price: result.currentPrice },
                Date.now() - startTime
            )
            return result
        } catch (error) {
        ...
        try {
            ...
            span?.update({
                metadata: {
                    ...
                    sentiment: newsData.overallSentiment ?? 'neutral',
                    responseTimeMs: Date.now() - startTime,
                },
            })
            span?.end()
            ...
            logStepEnd(
                'get-company-news',
                { symbol: inputData.symbol, newsCount: headlines.length },
                Date.now() - startTime
            )
            return { stockData: inputData, newsData }
        } catch (error) {
...
const runAnalysisStep = createStep({
    id: 'run-analysis',
    description:
        'Performs technical and fundamental analysis using stockAnalysisAgent',
    inputSchema: z.object({
        stockData: stockDataSchema,
        newsData: newsDataSchema,
    }),
    ...
    execute: async ({ inputData, mastra, writer, requestContext }) => {
        ...
        try {
            ...
            let technicalAnalysis: z.infer<
                typeof analysisDataSchema
            >['technicalAnalysis'] = {}
            let fundamentalAnalysis: z.infer<
                typeof analysisDataSchema
            >['fundamentalAnalysis'] = {}
        ...
        Provide technical and fundamental analysis in JSON format.`
            ...
            logStepEnd(
                'run-analysis',
                { symbol: result.symbol, trend: technicalAnalysis?.trend },
                Date.now() - startTime
            )
            return result
        } catch (error) {
    ...
    execute: async ({ inputData, mastra, writer, requestContext }) => {
        ...
        try {
         ...
         Provide a recommendation (strong-buy, buy, hold, sell, strong-sell), confidence score (0-100), price target, reasoning, and key risks.`
            ...
            logStepEnd(
                'generate-report',
                { symbol: result.symbol, recommendation },
                Date.now() - startTime
            )
            return result
        } catch (error) {
...
export const stockAnalysisWorkflow = createWorkflow({
    ...
    description:
        'Sequential stock analysis workflow with data enrichment at each step',
    inputSchema: z.object({
        symbol: z
            .string()
