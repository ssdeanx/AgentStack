...
const fetchWeather = createStep({
    ...
    execute: async (inputData: any, context: any) => {
        ...
        try {
            ...
            if (
                geocodingData.results === undefined ||
            ...
            span?.update({
                output: forecast,
                metadata: {
                    'weather.precipitationChance': forecast.precipitationChance,
...
const planActivities = createStep({
    ...
    execute: async (inputData: any, context: any) => {
        ...
        - All activities must be specific to location
        - Include specific venues, trails, or locations
        - Consider activity intensity based on temperature
        - Keep descriptions concise but informative

        Maintain this exact formatting for consistency, using emoji and section headers as shown.`
            ...
            const { fullStream } = response as unknown as {
                fullStream?: ReadableStream<unknown>
            }
            const { textStream } = response as unknown as {
            ...
            span?.update({
                output: result,
                metadata: {
                    'activities.length': activitiesText.length,
                },
            })
            span?.end()
            return result
