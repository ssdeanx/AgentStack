...
const retrievalStep = createStep({
    ...
    execute: async ({ inputData, mastra, requestContext }) => {
        ...
        try {
            ...
            log.info(`[${requestId}] 🔍 Tool results available:`, {
                toolResultsLength: retrieveResult.toolResults?.length || 0,
            })
            log.debug(`[${requestId}] 📋 Full retrieve result structure:`, {
                hasToolResults:
                ...
                toolResultsKeys: retrieveResult.toolResults?.map((tr) => ({
                ...
                hasText:
                    typeof retrieveResult.text === 'string' &&
                    retrieveResult.text.length > 0,
            ...
            if (
                ...
                const toolResultsArray = Array.isArray(
                ...
                if (toolResultData?.contexts) {
                    ...
                    log.warn(
                        `[${requestId}] ⚠️ Tool result found but no contexts property`,
                        {
                            ...
                            hasResult: toolResult?.payload?.result !== null,
            ...
            try {
                ...
                const output: {
                    contexts: Array<z.infer<typeof documentContextSchema>>
                    question: string
                } = {
