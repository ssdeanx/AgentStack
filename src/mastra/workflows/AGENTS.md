...
const calculateComfortScore = createStep({
    ...
    execute: async ({ inputData }) => {
        ...
        if (analysis.includes('sunny') || analysis.includes('clear'))
            score += 20
        if (analysis.includes('warm') || analysis.includes('comfortable'))
            score += 15
        if (analysis.includes('cool') || analysis.includes('pleasant'))
            score += 10
        ...
        if (analysis.includes('cold') || analysis.includes('freezing'))
            score -= 15
