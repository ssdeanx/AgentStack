...
const scanStep = createStep({
    ...
    execute: async ({ inputData, mastra, requestContext, writer }) => {
        ...
        if (githubRepoValue.length > 0) {
            log.info(
                `Scanning GitHub repo ${githubRepoValue} branch ${githubBranch}`
            )
            ...
            if (
                !isScanSuccess(scanResultRaw) ||
                !scanResultRaw.success ||
                !scanResultRaw.tree
            ) {
                const errorMessage = isScanSuccess(scanResultRaw)
                    ? (scanResultRaw.error ?? 'unknown error')
                    : 'unknown error'
            ...
            log.info(
                `Found ${tree.length} files in GitHub, processing ${files.length}`
            )
        ...
        if (repoPathValue.length > 0) {
            log.info(
                `Scanning local repo at ${repoPathValue} with pattern ${globPattern}`
            )
            ...
            log.info(
                `Found ${files.length} files, processing ${limitedFiles.length}`
            )
                    ...
                    try {
                        ...
                        if (githubRepoValue.length > 0) {
                            ...
                            if (result instanceof ZodError) {
                                throw new Error(
                                    `Validation error fetching file: ${result.message}`
                                )
                            }
                            ...
                            if (
                                ...
                                successResult.content === null ||
                                successResult.content === undefined ||
                                successResult.content === ''
                            ) {
                                throw new Error(
                                    `Failed to fetch file from GitHub: ${successResult.error}`
                                )
                            }
                        ...
                        let strategy: 'recursive' | 'markdown' | 'json' =
                            'recursive'
                        if (ext === '.md') {
                            strategy = 'markdown'
                        }
                        ...
                        const isSuccessResult = (
                            r: unknown
                        ): r is { success: true; chunkCount?: number } =>
                            typeof r === 'object' &&
                            r !== null &&
                            'success' in r &&
                        ...
                        log.error(`Error processing ${filePath}:`, {
                            error: errorObj,
                        })
                        errors.push({
                            file: filePath,
