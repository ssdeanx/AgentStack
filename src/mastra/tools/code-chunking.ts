        ...
        try {
            if (['ts', 'tsx', 'js', 'jsx'].includes(ext ?? '')) {
                ...
                if (chunks.length === 0 && content.trim().length > 0) {
                    ...
                    span?.update({
                        metadata: { 'tool.output.chunksCount': chunks.length },
                    })
                    span?.end()
                    return { chunks }
                }
            ...
            if (ext === 'py') {
                ...
                try {
                    ...
                    if (chunks.length > 0) {
                        ...
                        span?.update({
                            metadata: {
                                'tool.output.chunksCount': chunks.length,
                            },
                        })
                        span?.end()
                        return { chunks }
            ...
            if (abortSignal?.aborted) {
                span?.error({
                    error: new Error('Code chunking cancelled'),
                    endSpan: true,
                })
                await writer?.custom({
                    type: 'data-tool-progress',
            ...
            const textChunks = await doc.chunk({
                ...
                overlap:
                    options?.chunkOverlap ?? requestContext?.chunkOverlap ?? 50,
                separators: ['\n\n', '\n', ' '],
            })
            ...
            span?.update({
                metadata: { 'tool.output.chunksCount': mapped.length },
            })
            span?.end()
