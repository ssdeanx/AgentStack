...
const editOperationSchema = z.object({
    ...
    oldString: z
        .string()
        .describe('Exact text to find and replace (or regex pattern)'),
    newString: z.string().describe('Text to replace oldString with'),
    useRegex: z
        .boolean()
        .optional()
        .describe('Treat oldString as a regex pattern'),
    replaceAll: z
        .boolean()
        .optional()
...
const multiStringEditInputSchema = z.object({
    ...
    createBackup: z
        .boolean()
        .optional()
        .describe('Create .bak backup files before editing'),
    projectRoot: z
        .string()
        .optional()
        .describe('Project root for path validation (security boundary)'),
    maxFileSize: z
        .number()
        .optional()
...
Use for batch refactoring, multi-file updates, and coordinated code changes.`,
    ...
    execute: async (inputData, context) => {
        ...
        const requestContext =
            context?.requestContext as RequestContext<MultiStringEditContext>
        ...
        await writer?.custom({
            ...
            data: {
                status: 'in-progress',
                message: `🔁 Starting multi-string edit: ${inputData.edits.length} edits${inputData.dryRun === true ? ' (dry run)' : ''}`,
                stage: 'coding:multiStringEdit',
            },
            id: 'coding:multiStringEdit',
        })
        ...
        for (const edit of edits) {
            await writer?.custom?.({
                ...
                data: {
                    status: 'in-progress',
                    message: `✏️ Processing edit for: ${edit.filePath}`,
                    stage: 'coding:multiStringEdit',
                },
                id: 'coding:multiStringEdit',
            })
            const result = await processFileEdit(
                edit,
                defaultBoundary,
                dryRun,
                createBackup,
        ...
        await writer?.custom({
            ...
            data: {
                status: 'done',
                message: `✅ Multi-string edit complete: ${summary.applied} applied, ${summary.failed} failed, ${summary.skipped} skipped`,
                stage: 'coding:multiStringEdit',
            },
            id: 'coding:multiStringEdit',
        })
