    ...
    switch (def.type) {
        case 'string': {
            ...
            if (def.minLength !== undefined) {
                stringSchema = stringSchema.min(def.minLength)
            }
...
export const dataValidatorToolJSON = createTool({
    ...
    inputSchema: z.object({
        ...
        schema: z
            ...
            .describe('JSON schema definition (custom format)'),
    }),
    ...
    onInputDelta: ({ inputTextDelta, toolCallId, messages, abortSignal }) => {
        log.info('Data validator received input chunk', {
            toolCallId,
            inputTextDelta,
            hook: 'onInputDelta',
        })
    ...
    execute: async (inputData, context) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                const { maxErrors } =
                    config !== undefined
                        ? validatorContextSchema.parse(config)
                        : { maxErrors: undefined }
                ...
                if (maxErrors !== undefined && errors.length > maxErrors) {
                    ...
                    errors.push(
                        `...and ${result.error.issues.length - maxErrors} more errors`
                    )
                }
