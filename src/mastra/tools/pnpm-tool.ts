...
export const pnpmBuild = createTool({
    ...
    onInputStart: ({ toolCallId, messages, abortSignal }) => {
        log.info('pnpmBuild tool input streaming started', {
            toolCallId,
            hook: 'onInputStart',
        })
    },
    ...
    execute: async (inputData, context) => {
        ...
        if (verbose) {
            await writer?.custom({
                ...
                data: {
                    status: 'in-progress',
                    message: `🔨 Building ${name} at ${packagePath}`,
                    stage: 'pnpmBuild',
                },
                id: 'pnpmBuild',
            })
        ...
        try {
            ...
            if (verbose) {
                await writer?.custom({
                    ...
                    data: {
                        status: 'done',
                        message: `✅ Build complete for ${name}`,
                        stage: 'pnpmBuild',
                    },
                    id: 'pnpmBuild',
                })
...
export const pnpmChangesetStatus = createTool({
    ...
    onInputStart: ({ toolCallId, messages, abortSignal }) => {
        log.info('pnpmChangesetStatus tool input streaming started', {
            toolCallId,
            hook: 'onInputStart',
        })
    },
    ...
    execute: async (inputData, context) => {
        ...
        await writer?.custom({
            ...
            data: {
                status: 'in-progress',
                message: '🔍 Checking changeset status...',
                stage: 'pnpmChangesetStatus',
            },
            id: 'pnpmChangesetStatus',
        })
        try {
            ...
            await writer?.custom({
                ...
                data: {
                    status: 'done',
                    message: `✅ Found ${packages.length} packages to publish`,
                    stage: 'pnpmChangesetStatus',
                },
                id: 'pnpmChangesetStatus',
            })
            ...
            await writer?.custom({
                ...
                data: {
                    status: 'done',
                    message: `🔍 Changeset check failed: ${errorMsg}`,
                    stage: 'pnpmChangesetStatus',
                },
                id: 'pnpmChangesetStatus',
            })
...
export const pnpmChangesetPublish = createTool({
    ...
    onInputStart: ({ toolCallId, messages, abortSignal }) => {
        log.info('pnpmChangesetPublish tool input streaming started', {
            toolCallId,
            hook: 'onInputStart',
        })
    },
    ...
    execute: async (inputData, context) => {
        ...
        await writer?.custom({
            ...
            data: {
                status: 'in-progress',
                message: '🚀 Publishing changesets...',
                stage: 'pnpmChangesetPublish',
            },
            id: 'pnpmChangesetPublish',
        })
        try {
...
export const activeDistTag = createTool({
    ...
    onInputStart: ({ toolCallId, messages, abortSignal }) => {
        log.info('activeDistTag tool input streaming started', {
            toolCallId,
            hook: 'onInputStart',
        })
    },
    ...
    execute: async (input, context) => {
        ...
        await writer?.custom({
            ...
            data: {
                status: 'in-progress',
                message: `🏷️ Setting active dist tag for ${packagePath}`,
                stage: 'activeDistTag',
            },
            id: 'activeDistTag',
        })
        try {
            ...
            await writer?.custom({
                ...
                data: {
                    status: 'done',
                    message: `✅ Dist tag set to latest for ${pkgJson.name}@${version}`,
                    stage: 'activeDistTag',
                },
                id: 'activeDistTag',
            })
            ...
            await writer?.custom({
                ...
                data: {
                    status: 'done',
                    message: `🚨 Dist tag failed: ${errorMsg}`,
                    stage: 'activeDistTag',
                },
                id: 'activeDistTag',
            })
...
export const pnpmRun = createTool({
    ...
    onInputStart: ({ toolCallId, messages, abortSignal }) => {
        log.info('pnpmRun tool input streaming started', {
            toolCallId,
            hook: 'onInputStart',
        })
    },
    ...
    execute: async (input, context) => {
        ...
        await writer?.custom({
            ...
            data: {
                status: 'in-progress',
                message: `🏃 Running pnpm ${script} ${args.join(' ')}`,
                stage: 'pnpmRun',
            },
            id: 'pnpmRun',
        })
        try {
            ...
            await writer?.custom({
                ...
                data: {
                    status: 'done',
                    message: `✅ Script ${script} complete`,
                    stage: 'pnpmRun',
                },
                id: 'pnpmRun',
            })
            ...
            await writer?.custom({
                ...
                data: {
                    status: 'done',
                    message: `❌ Script failed: ${errorMsg}`,
                    stage: 'pnpmRun',
                },
                id: 'pnpmRun',
            })
