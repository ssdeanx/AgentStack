  ...
  execute: async (inputData, context) => {
    ...
    try {
      ...
      if (existingUrls?.includes(result.url)) {
        evalSpan.end();
        await context?.writer?.custom({ type: 'data-tool-progress', data: { status: 'done', message: 'URL already processed', stage: 'evaluate-result' }, id: 'evaluate-result' });
        return {
          isRelevant: false,
      ...
      const agent = context?.mastra?.getAgent?.('evaluationAgent') ?? evaluationAgent;

      let responseObject: unknown = {}
      if (typeof agent.stream === 'function') {
        try {
          await context?.writer?.custom({ type: 'data-tool-progress', data: { status: 'in-progress', message: '🔁 Streaming evaluation from agent', stage: 'evaluate-result' }, id: 'evaluate-result' });
          // Use MastraModelOutput for accurate typing and pipe the appropriate stream into the writer
          const stream = await agent.stream(`Evaluate whether this search result is relevant and will help answer the query: "${query}".\n\nSearch result:\nTitle: ${result.title}\nURL: ${result.url}\nContent snippet: ${result.content.substring(0, 500)}...\n\nRespond with a JSON object containing:\n- isRelevant: boolean indicating if the result is relevant\n- reason: brief explanation of your decision`) as MastraModelOutput | undefined

          if (stream?.textStream && context?.writer) {
            await context?.writer?.custom({ type: 'data-tool-progress', data: { status: 'in-progress', message: '🔁 Streaming text from evaluation agent', stage: 'evaluate-result' }, id: 'evaluate-result' });
            await stream.textStream.pipeTo(context.writer as unknown as WritableStream)
          } else if (stream?.fullStream && context?.writer) {
            await context?.writer?.custom({ type: 'data-tool-progress', data: { status: 'in-progress', message: '🔁 Streaming from evaluation agent (full stream)', stage: 'evaluate-result' }, id: 'evaluate-result' });
            await stream.fullStream.pipeTo(context.writer as unknown as WritableStream)
          }
          ...
          await context?.writer?.custom({ type: 'data-tool-progress', data: { status: 'done', message: `❌ Error streaming evaluation: ${errorMessage}`, stage: 'evaluate-result' }, id: 'evaluate-result' });
          return {
            isRelevant: false,
        ...
        const response = await agent.generate(`Evaluate whether this search result is relevant and will help answer the query: "${query}".\n\nSearch result:\nTitle: ${result.title}\nURL: ${result.url}\nContent snippet: ${result.content.substring(0, 500)}...\n\nRespond with a JSON object containing:\n- isRelevant: boolean indicating if the result is relevant\n- reason: brief explanation of your decision`)
        responseObject = response.object ?? (() => { try { return JSON.parse(response.text) } catch { return {} } })()
      }
      ...
      if (!parsed.success) {
        ...
        evalSpan.end();
        await context?.writer?.custom({ type: 'data-tool-progress', data: { status: 'done', message: error, stage: 'evaluate-result' }, id: 'evaluate-result' });
        return {
          isRelevant: false,
          reason: error,
        }
      ...
      await context?.writer?.custom({ type: 'data-tool-progress', data: { status: 'done', message: `❌ Error evaluating result: ${errorMessage}`, stage: 'evaluate-result' }, id: 'evaluate-result' });
