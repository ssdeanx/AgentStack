...
const findReferencesInputSchema = z.object({
    ...
    filePath: z
        .string()
        .optional()
        .describe('File path where the symbol is defined (for precise lookup)'),
    line: z
        .number()
        .optional()
        .describe('Line number of the symbol definition (for precise lookup)'),
...
export const findReferencesTool = createTool({
    ...
    execute: async (inputData, context) => {
        ...
        const { maxReferences, includeNodeModules, caseSensitive } =
            referenceContextSchema.parse(refContext ?? {})
        ...
        try {
            ...
            await writer?.custom({
                ...
                data: {
                    status: 'in-progress',
                    message: `🧭 Starting TypeScript/JavaScript analysis`,
                    stage: 'semantic:find-references',
                },
                id: 'semantic:find-references',
            })
            ...
            if (
                ...
                await writer?.custom({
                    ...
                    data: {
                        status: 'in-progress',
                        message: `📌 Precise lookup: ${filePath}:${line}`,
                        stage: 'semantic:find-references',
                    },
                    id: 'semantic:find-references',
                })
                ...
                if (sourceFile) {
                    const position =
                        sourceFile.compilerNode.getPositionOfLineAndCharacter(
                            line - 1,
                            0
                        )
                    ...
                    if (node) {
                        ...
                        if (symbol) {
                            const references = project
                                .getLanguageService()
                                .findReferencesAtPosition(sourceFile, position)
                            for (const ref of references ?? []) {
                                for (const reference of ref.getReferences()) {
                                    ...
                                    const pos =
                                        refSourceFile.getLineAndColumnAtPos(
                                            start
                                        )
                ...
                await writer?.custom({
                    ...
                    data: {
                        status: 'in-progress',
                        message: `📁 Name-based search across project files...`,
                        stage: 'semantic:find-references',
                    },
                    id: 'semantic:find-references',
                })
                ...
                for (const sourceFile of sourceFiles) {
                    ...
                    if (
                        sourceFilePath.includes('.git') ||
                        sourceFilePath.includes('dist') ||
                        sourceFilePath.includes('build')
                    ) {
                        continue
                    }
                    ...
                    try {
                        const fileReferences =
                            await analyzeTypeScriptReferences(
                                ...
                                searchTerm,
                                caseSensitive,
                                maxReferences - allReferences.length
                            )
                        await writer?.custom({
                            type: 'data-tool-progress',
                            data: {
                                status: 'in-progress',
                                message: `📄 Scanned: ${sourceFile.getFilePath()} (${fileReferences.length} matches)`,
                                stage: 'semantic:find-references',
                            },
                            id: 'semantic:find-references',
                        })
                        ...
                        log.warn(`Error analyzing file ${sourceFilePath}`, {
                            error,
                        })
                    }
                }
            ...
            await writer?.custom({
                ...
                data: {
                    status: 'in-progress',
                    message: `🐍 Starting Python analysis`,
                    stage: 'semantic:find-references',
                },
                id: 'semantic:find-references',
            })
            try {
                ...
                await writer?.custom({
                    ...
                    data: {
                        status: 'in-progress',
                        message: `🐍 Python files to check: ${pythonFiles.length}`,
                        stage: 'semantic:find-references',
                    },
                    id: 'semantic:find-references',
                })
                for (const pyFile of pythonFiles) {
                    try {
                        ...
                        const references = await PythonParser.findReferences(
                            content,
                            symbolName
                        )
                        ...
                        if (references.length > 0) {
                            await writer?.custom({
                                ...
                                data: {
                                    status: 'in-progress',
                                    message: `🐍 Found ${references.length} matches in ${pyFile}`,
                                    stage: 'semantic:find-references',
                                },
                                id: 'semantic:find-references',
                            })
            ...
            const filesCount = new Set(allReferences.map((r) => r.filePath))
                .size
            ...
            await writer?.custom({
                ...
                data: {
                    status: 'done',
                    message: `✅ Find references complete: ${allReferences.length} references across ${filesCount} files`,
                    stage: 'semantic:find-references',
                },
                id: 'semantic:find-references',
            })
            ...
            await writer?.custom({
                ...
                data: {
                    status: 'done',
                    message: `❌ Find references failed: ${errorMessage}`,
                    stage: 'semantic:find-references',
                },
                id: 'semantic:find-references',
            })
