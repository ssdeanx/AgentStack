...
export class ProjectCache {
    ...
    public getOrCreate(projectPath: string): Project {
        ...
        try {
            ...
            const estimatedMemoryMB = Math.max(
                fileCount * this.MEMORY_PER_FILE_MB,
                10
            ) // Minimum 10MB
            ...
            if (estimatedMemoryMB <= this.MAX_PROJECT_MEMORY_MB) {
                ...
                log.info(
                    `Cached project ${normalizedPath}: ${fileCount} files, ~${estimatedMemoryMB.toFixed(1)}MB`
                )
            } else {
                log.warn(
                    `Project ${normalizedPath} too large (${estimatedMemoryMB.toFixed(1)}MB) - skipping cache`
                )
            }

            return project
        } catch (error) {
            log.error(`Failed to create project for ${normalizedPath}`, {
                error,
            })
            throw new Error(
...
`
    ...
    private static async getPythonCommand(): Promise<string> {
        ...
        throw new Error(
            'Python 3 not found. Please install Python 3 to analyze Python code.'
        )
    }
    ...
    private static async ensureScriptExists(): Promise<string> {
        ...
        this.scriptPath = path.join(
            os.tmpdir(),
            `mastra-python-parser-${process.pid}.py`
        )
    ...
    private static async executePython(
        ...
        try {
            ...
            const result = await new Promise<PythonResult>(
                (resolve, reject) => {
                    const child = spawn(
                        pythonCmd,
                        [scriptPath, action, ...args],
                        {
                    ...
                    child.on('close', () => {
                        ...
                        try {
                            ...
                            reject(
                                new Error(
                                    `Failed to parse Python output: ${stdout.substring(0, 200)}`
                                )
                            )
                        }
                    })
    ...
    public static async analyzeComplexity(
        code: string
    ...
    > {
        const result = await this.executePython(code, 'references', [
            symbolName,
        ])
