        ...
        try {
            ...
            span?.update({
                metadata: {
                    ...
                    'file.size': inputData.content.length,
                },
            })
            span?.end()
    ...
    execute: async (inputData, context) => {
        ...
        const span = getOrCreateSpan({
            ...
            metadata: {
                'tool.id': 'writeFiles',
                'tool.input.sandboxId': inputData.sandboxId,
                'tool.input.fileCount': Array.isArray(inputData.files)
                    ? inputData.files.length
                    : undefined,
            },
        })
        const startTime = Date.now()
        ...
        try {
            ...
            const totalSize = inputData.files.reduce(
            ...
            span?.update({
                metadata: {
                    'operation.status': 'success',
                    'sandbox.id': inputData.sandboxId,
                    'files.count': inputData.files.length,
        ...
        try {
            ...
            span?.update({
                metadata: {
                    ...
                    'file.path': inputData.path,
                    'file.size': info.size,
                    humanReadable: inputData.humanReadable,
                },
            })
            if (info.type !== null && info.type !== undefined) {
                span?.update({ metadata: { 'file.type': info.type } })
...
export const watchDirectory = createTool({
    ...
    inputSchema: z.object({
        ...
        path: z.string().describe('The directory path to watch for events'),
        recursive: z
            .boolean()
            .default(false)
        ...
        try {
            ...
            span?.update({
                metadata: {
                    'operation.status': 'success',
                    'sandbox.id': inputData.sandboxId,
                    'watch.path': inputData.path,
                    'watch.recursive': inputData.recursive,
                    'watch.duration': inputData.watchDuration,
        ...
        try {
            ...
            span?.update({
                metadata: {
                    'operation.status': 'success',
                    'sandbox.id': inputData.sandboxId,
                    'command.text': inputData.command,
                    'command.cwd': inputData.workingDirectory ?? '/',
                    'command.exit_code': result.exitCode,
                    'command.execution_time': executionTime,
                    'command.stdout_length': (result.stdout ?? '').length,
                    'command.stderr_length': (result.stderr ?? '').length,
                },
            })
...
export const runCode = createTool({
    ...
    outputSchema: z.object({
        execution: z
            .string()
            .describe('JSON string containing execution results'),
    }),
    execute: async (inputData, context) => {
        const tracingContext = context?.tracingContext
        const span = getOrCreateSpan({
            ...
            metadata: {
                ...
                'tool.input.language':
                    inputData.runCodeOpts?.language ?? 'python',
            },
        })
        const startTime = Date.now()
        ...
        try {
            ...
            span?.update({
                metadata: {
                    ...
                    'code.length': inputData.code.length,
                    'command.exit_code': result.exitCode,
                    'command.stdout_length': (result.stdout ?? '').length,
                    'command.stderr_length': (result.stderr ?? '').length,
                },
