...
const testGeneratorInputSchema = z.object({
  sourceFile: z
    .string()
    .describe('Path to the source file to generate tests for'),
  outputPath: z
    .string()
    .optional()
    .describe('Output path for test file (defaults to __tests__ folder)'),
  options: z
    .object({
      includeEdgeCases: z
        .boolean()
        .default(true)
        .describe('Generate edge case tests'),
      mockExternals: z
        .boolean()
        .default(true)
        .describe('Add mock setup for external dependencies'),
      testStyle: z
        .enum(['describe-it', 'test-only'])
        .default('describe-it')
        .describe('Test structure style'),
    })
    .optional(),
})
...
const testGeneratorOutputSchema = z
  .object({
    testFile: z.string(),
    sourceFile: z.string(),
...
function parseExports(content: string): string[] {
  ...
  while ((exportMatch = defaultExportRegex.exec(content)) !== null) {
    if (exportMatch[1]) {
      exports.push(`default (${exportMatch[1]})`)
    } else {
...
Use for increasing test coverage and establishing testing patterns.`,
  ...
  execute: async (inputData, context?) => {
    ...
    const span = tracingContext?.currentSpan?.createChildSpan({
      ...
      metadata: {
        'tool.id': 'coding:testGenerator',
        'tool.input.sourceFile': sourceFile,
        'tool.input.testStyle': testStyle,
        'tool.input.operation': 'generate-tests',
        'user.id': userId,
    ...
    try {
      ...
      await context?.writer?.custom({
        ...
        data: {
          status: 'in-progress',
          message: `🧪 Generating tests for: ${sourceFile}`,
          stage: 'coding:testGenerator',
        },
        id: 'coding:testGenerator',
      })
      ...
      const testFile =
        outputPath ??
        path.join(sourceDir, '__tests__', `${sourceName}.test.ts`)
      ...
      try {
        span?.update({
          output: result,
          metadata: {
            'tool.output.testCount': tests.length,
            'tool.output.testFile': testFile
          },
        })
      } catch {
