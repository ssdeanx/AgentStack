...
function convertPdfTextToMarkdown(text: string, metadata?: { title?: string; authors?: string[] }): string {
  ...
  if (hasTitle || hasAuthors) {
    const frontmatter = [
      '---',
      hasTitle ? `title: "${String(metadata?.title).replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"` : null,
  ...
  execute: async ({ context, runtimeContext, writer, tracingContext }) => {
    ...
    try {
      ...
      if (context.normalizeText) {
        ...
        const frontmatter = [
          '---',
          (typeof paperMetadata.title === 'string' && paperMetadata.title.trim() !== '') ? `title: "${paperMetadata.title.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"` : null,
