  ...
  execute: async (inputData, context) => {
    ...
    const toolSpan = tracingContext?.currentSpan?.createChildSpan({
      ...
      metadata: {
        ...
        'tool.input.saveMarkdown': inputData.storage?.saveMarkdown ?? false,
        'tool.input.followLinks': inputData.crawling?.followLinks ?? false,
        'user.id': userId,
    ...
    try {
            ...
            try {
              ...
              if (inputData.extraction?.includeImages ?? false) {
                ...
                imgElements.forEach((img) => {
                  ...
                  if (trimmedSrc !== '') {
                    images.push({
                      src: new URL(trimmedSrc, request.url)
                        .href,
                      alt:
              ...
              if (
                ...
                microdataElements.forEach((element) => {
                  ...
                  if (trimmedItemType !== '') {
                    ...
                    const props =
                      element.querySelectorAll('[itemprop]')
                    props.forEach((prop) => {
                      const propName =
              ...
              try {
                contentSpan?.update({
                  metadata: {
                    ...
                    'operation.metadataCount': Object.keys(metadata).length,
                    'operation.imagesCount': images.length,
                    'operation.structuredDataCount': structuredData.length,
                    'operation.detectedLanguage': detectedLanguage ?? undefined,
                  },
                })
              } catch {
                // ignore tracing update errors
      ...
      if (
        ...
        try {
          ...
          try {
            convertSpan?.update({ metadata: { 'operation.status': 'primary_failed' } })
          } catch {
            // ignore tracing update errors
          }
      ...
      if (
        ...
        try {
            ...
            try {
              saveSpan?.update({
                metadata: {
                  ...
                  'result.savedFileSize': typeof markdownContent === 'string' ? markdownContent.length : 0,
                },
              })
            } catch {
              // ignore tracing update errors
          ...
          try {
            saveSpan?.error({ error: error instanceof Error ? error : new Error(String(error)), endSpan: true })
          } catch {
            // ignore tracing errors
          }
          if (error instanceof ScrapingError) {
      ...
      toolSpan?.update({
        ...
        metadata: {
          ...
          'tool.output.markdownLength': typeof markdownContent === 'string' ? markdownContent.length : 0,
