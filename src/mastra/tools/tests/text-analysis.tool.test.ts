...
describe('textAnalysisTool', () => {
    ...
    it('should count words, sentences, paragraphs', async () => {
        ...
        const res = await textAnalysisTool.execute({
            text,
            operations: ['word-count', 'sentence-count', 'paragraph-count'],
        })
    ...
    it('should calculate readability and detect language', async () => {
        ...
        const res = await textAnalysisTool.execute({
            text: en,
            operations: ['readability', 'language-detect'],
        })
        expect(res.success).toBe(true)
    ...
    it('should do sentiment with advanced metrics disabled and enabled', async () => {
        ...
        const res1 = await textAnalysisTool.execute({
            text,
        ...
        const res2 = await textAnalysisTool.execute(
            { text, operations: ['sentiment'] },
            { requestContext: { includeAdvancedMetrics: true } }
        )
