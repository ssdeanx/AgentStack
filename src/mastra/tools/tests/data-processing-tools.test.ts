...
describe('data-processing-tools', () => {
    ...
    it('converts json to csv and back', async () => {
        ...
        const jsonToCsv = await convertDataFormatTool.execute({
            inputData: data,
            inputFormat: 'json',
            outputFormat: 'csv',
        })
        ...
        const csvToJson = await convertDataFormatTool.execute({
            inputData: csv,
            inputFormat: 'csv',
            outputFormat: 'json',
        })
