...
vi.mock('../semantic-utils', () => ({
    ProjectCache: {
        getInstance: () => ({
            getOrCreate: () => ({ getSourceFiles: () => [] }),
        }),
    },
    PythonParser: {
        findReferences: async (content: string, symbolName: string) => [
            { line: 1, column: 2, text: symbolName, isDefinition: false },
        ],
    },
}))
...
describe('findReferencesTool', () => {
    ...
    it('includes python references in results', async () => {
        const res = await findReferencesTool.execute({
            symbolName: 'myfunc',
