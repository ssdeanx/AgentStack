...
describe('randomGeneratorTool', () => {
    it('generates a number within range', async () => {
        const res = await randomGeneratorTool.execute({
            type: 'number',
            count: 1,
    ...
    it('generates strings of correct length and special chars', async () => {
        const res = await randomGeneratorTool.execute({
            type: 'string',
            count: 1,
            options: { length: 12, includeSpecial: true },
        })
        expect(res.success).toBe(true)
    ...
    it('generates email and name', async () => {
        const email = await randomGeneratorTool.execute({
            type: 'email',
            count: 1,
        })
        ...
        const name = await randomGeneratorTool.execute({
            type: 'name',
            count: 1,
            options: { locale: 'es' },
        })
        expect(typeof name.data).toBe('string')
        expect((name.data as string).split(' ').length).toBeGreaterThanOrEqual(
            2
        )
    })
})
