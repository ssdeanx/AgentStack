...
Use for finding usages, identifying patterns, and code exploration.`,
    ...
    execute: async (
        ...
        try {
            ...
            const caseSensitive =
                options?.caseSensitive ?? requestContext?.caseSensitive ?? false
            const maxResults =
                options?.maxResults ?? requestContext?.maxResults ?? 100
            const includeContext =
                options?.includeContext ??
                requestContext?.includeContext ??
                true
            const contextLines =
                options?.contextLines ?? requestContext?.contextLines ?? 2
            const maxFileSize =
                options?.maxFileSize ?? requestContext?.maxFileSize ?? 1_000_000
