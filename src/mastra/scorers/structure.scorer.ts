...
.analyze(async ({ results, run }) => {
    ...
    interface ContextType { expectedKeys?: string[], expectedSections?: string[] }
    const context = (run as unknown as { context: ContextType }).context
    const {expectedKeys} = context
    const {expectedSections} = context
