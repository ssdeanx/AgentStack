...
.generateReason(({ results, score }) => {
    ...
    // Include a concise domain breakdown to better explain the score
    const domainEntries = Object.entries(domainBreakdown)
    const domainSummary = domainEntries.length > 0
        ? domainEntries.map(([domain, count]) => `${domain}: ${count}`).join(', ')
        : 'No valid domains parsed'

    reason += ` Domain breakdown: ${domainSummary}.`
...
.generateReason(({ results, score }) => {
    ...
    reason += ` Included ${learningCount} learnings.`
...
.preprocess(({ run }) => {
    ...
    if (typeof output === 'string') {
        try {
            const parsed = JSON.parse(output) as { summary?: string; learnings?: Learning[]; data?: string }
            summary = parsed.summary ?? ''
            learnings = parsed.learnings ?? []
            data = parsed.data ?? ''
        } catch {
        ...
        const typed = output as { summary?: string; learnings?: Learning[]; data?: string }
        summary = typed.summary ?? ''
        learnings = typed.learnings ?? []
        data = typed.data ?? ''
    }
...
.analyze(({ results }) => {
    ...
    const keyInsights = learnings.filter((learning: Learning) => {
        const insight = (learning.insight ?? '').toLowerCase()
        return insight.length > 10 && summaryLower.includes(insight.slice(0, 20))
    }).length

    // Check if the underlying data section supports the key learnings
    const dataInsights = learnings.filter((learning: Learning) => {
        const insight = (learning.insight ?? '').toLowerCase()
        return insight.length > 10 && dataLower.includes(insight.slice(0, 20))
    }).length

    const insightCoverage = learnings.length > 0 ? keyInsights / learnings.length : 0
    const dataSupportCoverage = learnings.length > 0 ? dataInsights / learnings.length : 0
    ...
    // Incorporate how much of the learnings are supported by explicit data
    const qualityScore = (insightCoverage * 0.55) + (dataSupportCoverage * 0.15) + (characteristicsMet / totalCharacteristics * 0.3)
...
.preprocess(({ run }) => {
    ...
    if (typeof output === 'string') {
        responseText = output
    } else if ((Boolean(output)) && typeof output === 'object') {
        responseText = JSON.stringify(output)
    }
