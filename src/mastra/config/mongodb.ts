...
> {
    ...
    try {
        ...
        const processingTime = Date.now() - startTime
        log.error('Failed to query similar documents', {
            error: String(error),
            processingTimeMs: processingTime,
        })
...
export const mongoGraphTool = createGraphRAGTool({
    ...
    graphOptions: {
        ...
        randomWalkSteps: parseInt(
            process.env.MONGO_GRAPH_RANDOM_WALK_STEPS ?? '10'
        ),
 ...
 */
export function transformToMongoDBFilter(
    filter: MongoDBMetadataFilter
): MongoDBRawFilter {
 ...
 */
export function validateMongoDBFilter(
    filter: MongoDBMetadataFilter
): MongoDBMetadataFilter {
