...
export class ModelRegistry {
    ...
    registerModel(metadata: ModelMetadata, instance?: LanguageModel) {
        try {
            ...
            // Explicitly handle provider not registered -- mark model as unavailable and log
            let isAvailable = true
            if (!providerCfg) {
                // If provider wasn't registered before model registration, mark it unavailable.
                // This signals to callers that the model is not ready for use.
                log.info(`Provider not registered for model ${metadata.id}`, { provider: metadata.provider })
                isAvailable = false
            } else {
            ...
            this.models.set(metadata.id, registered)
                if (!providerCfg) {
                    // Offer quick discovery for maintainers running locally: log a small hint so they can
                    // register the provider or set the env var used for configuration.
                    log.info(`Registered model ${metadata.id} but provider ${metadata.provider} is not registered - set env var in .env or import provider config earlier`, { id: metadata.id })
                }
    ...
    // Return all models, including those that are not currently available.
    getAllModels() {
    ...
    // Test helper: clears models and providers for deterministic test fixtures.
    // Only allowed in test environment to avoid accidental data loss during runtime.
    clearForTests() {
