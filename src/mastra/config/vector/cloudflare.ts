...
const CLOUDFLARE_STORAGE_CONFIG = {
    ...
    // Connection settings
    baseUrl:
        process.env.CLOUDFLARE_D1_BASE_URL ??
...
export async function initializeCloudflareStorage(): Promise<void> {
    try {
        ...
        if (
            CLOUDFLARE_STORAGE_CONFIG.binding &&
            CLOUDFLARE_STORAGE_CONFIG.binding.trim() !== ''
        ) {
        ...
        log.error('Failed to initialize Cloudflare storage', {
            error: String(error),
        })
        throw error
    }
...
export const cloudflareMemory = new Memory({
    ...
    options: {
        // Message management
        lastMessages: parseInt(
            process.env.CLOUDFLARE_MEMORY_LAST_MESSAGES ?? '500'
        ),
        ...
        semanticRecall: {
            ...
            messageRange: {
                before: parseInt(
                    process.env.CLOUDFLARE_SEMANTIC_RANGE_BEFORE ?? '3'
                ),
                after: parseInt(
                    process.env.CLOUDFLARE_SEMANTIC_RANGE_AFTER ?? '2'
                ),
            },
      ...
      `,
        ...
        threads: {
            generateTitle:
                process.env.CLOUDFLARE_THREAD_GENERATE_TITLE !== 'false',
        },
    },
...
export async function initializeDatabase(): Promise<void> {
    try {
        ...
        log.error('Failed to initialize Cloudflare database', {
            error: String(error),
        })
        throw error
    }
...
export async function shutdownDatabase(): Promise<void> {
    try {
        ...
        log.error('Error during Cloudflare database shutdown', {
            error: String(error),
        })
        throw error
    }
...
export function formatStorageMessages(
    ...
    return {
        ...
        result: result
            ? maskSensitiveMessageData(result as Record<string, unknown>)
            : undefined,
    }
}
