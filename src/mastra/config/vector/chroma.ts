...
const CHROMA_CONFIG = {
    ...
    embeddingDimension: parseInt(
        process.env.CHROMA_EMBEDDING_DIMENSION ?? '1536'
    ),
...
const chromaStore = new ChromaVector(
    CHROMA_CONFIG.apiKey !== null &&
        CHROMA_CONFIG.apiKey !== undefined &&
        CHROMA_CONFIG.apiKey.trim() !== '' &&
        CHROMA_CONFIG.tenant !== null &&
        CHROMA_CONFIG.tenant !== undefined &&
        CHROMA_CONFIG.tenant.trim() !== '' &&
        CHROMA_CONFIG.database !== null &&
        CHROMA_CONFIG.database !== undefined &&
        CHROMA_CONFIG.database.trim() !== ''
        ? {
...
export async function initializeVectorIndex(): Promise<void> {
    try {
        ...
        if (
            (errorObj.message?.includes('already exists') ?? false) ||
            errorObj.code === 'index_already_exists'
        ) {
...
export async function storeDocumentEmbeddings(
    ...
    try {
        ...
        log.error('Failed to store document embeddings', {
            error: String(error),
        })
        throw error
    }
 ...
 */
export function transformToChromaFilter(
...
export const chromaGraphTool = createGraphRAGTool({
    ...
    graphOptions: {
        ...
        randomWalkSteps: parseInt(
            process.env.CHROMA_GRAPH_RANDOM_WALK_STEPS ?? '10'
        ),
        restartProb: parseFloat(
            process.env.CHROMA_GRAPH_RESTART_PROB ?? '0.15'
        ),
    },
    includeSources: true,
