...
const COUCHBASE_CONFIG = {
    ...
    embeddingDimension: parseInt(
        process.env.COUCHBASE_EMBEDDING_DIMENSION ?? '1536'
    ),
 ...
 */
export function transformToCouchbaseFilter(
 ...
 */
export function validateCouchbaseFilter(
    filter: CouchbaseMetadataFilter
): boolean {
    try {
...
export const couchbaseGraphTool = createGraphRAGTool({
    ...
    graphOptions: {
        ...
        randomWalkSteps: parseInt(
            process.env.COUCHBASE_GRAPH_RANDOM_WALK_STEPS ?? '10'
        ),
        restartProb: parseFloat(
            process.env.COUCHBASE_GRAPH_RESTART_PROB ?? '0.15'
        ),
    },
    includeSources: true,
