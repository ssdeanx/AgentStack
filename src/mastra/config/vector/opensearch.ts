...
const OPENSEARCH_CONFIG = {
    ...
    embeddingDimension: parseInt(
        process.env.OPENSEARCH_EMBEDDING_DIMENSION ?? '1536'
    ),
 ...
 */
export function transformToOpenSearchFilter(
 ...
 */
export function validateOpenSearchFilter(
    filter: OpenSearchMetadataFilter
): boolean {
    try {
...
export const openSearchGraphTool = createGraphRAGTool({
    ...
    graphOptions: {
        ...
        randomWalkSteps: parseInt(
            process.env.OPENSEARCH_GRAPH_RANDOM_WALK_STEPS ?? '10'
        ),
        restartProb: parseFloat(
            process.env.OPENSEARCH_GRAPH_RESTART_PROB ?? '0.15'
        ),
    },
    includeSources: true,
