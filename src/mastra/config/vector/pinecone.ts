 ...
 */
export function transformToPineconeFilter(
 ...
 */
export function validatePineconeFilter(
    filter: PineconeMetadataFilter
): boolean {
    try {
...
export const pineconeGraphTool = createGraphRAGTool({
    ...
    graphOptions: {
        ...
        randomWalkSteps: parseInt(
            process.env.PINECONE_GRAPH_RANDOM_WALK_STEPS ?? '10'
        ),
        restartProb: parseFloat(
            process.env.PINECONE_GRAPH_RESTART_PROB ?? '0.15'
        ),
    },
    includeSources: true,
