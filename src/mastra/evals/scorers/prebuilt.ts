...
export function createCompletenessScorer() {
    return createScorer({
        ...
        name: 'Completeness',
        description: 'Evaluates coverage of input elements',
        type: 'agent',
    })
        .preprocess(({ run }) => {
        ...
        .analyze(({ results }) => {
            ...
            const missingElements = inputElements.filter(
                (t) => !outputElements.includes(t)
            )
            return {
                inputElements,
                outputElements,
        ...
        .generateScore(({ results }) => {
            ...
            const covered = inputTerms.filter((t) =>
                outputTerms.includes(t)
            ).length
...
export function createToolCallAccuracyScorerCode(
    ...
    return createScorer({
        id: 'tool-call-accuracy-code',
        name: 'Tool Call Accuracy (code)',
        description: 'Deterministic tool call accuracy scorer',
    })
        .preprocess(({ run }) => {
        ...
        .generateScore(({ results }) => {
            const preprocess = results.preprocessStepResult ?? {
                tools: [],
                opts: {},
            }
            const tools = Array.isArray(preprocess.tools)
                ? preprocess.tools
                : []
            const { expectedTool, strictMode, expectedToolOrder } =
                preprocess.opts ?? {}
...
export function createToolCallAccuracyScorerLLM() {
    ...
    return createScorer({
        id: 'tool-call-accuracy-llm',
        name: 'Tool Call Accuracy (LLM)',
        description: 'LLM-based tool call appropriateness scorer',
        judge: undefined,
    })
}
...
export function createNoiseSensitivityScorerLLM(
    ...
    return createScorer({
        ...
        description: 'Evaluates robustness to noise',
        type: 'agent',
    })
        .preprocess(({ run }) => {
            const input =
        ...
        .generateScore(({ results }) => {
            ...
            const outputText = results.preprocessStepResult?.outputText ?? ''
            const baseline =
                typeof opts.baselineResponse === 'string'
            ...
            if (typeof baseline === 'string' && baseline.trim().length > 0) {
                ...
                return outputText.trim() === baselineTrim
                    ...
                    : Math.max(
                          0,
                          1 -
                              levenshtein(outputText, baselineTrim) /
                                  Math.max(
                                      outputText.length,
                                      baselineTrim.length,
                                      1
...
export function createTextualDifferenceScorer() {
    return createScorer({
        ...
        description:
            'Measures textual changes required between expected and actual text',
        type: 'agent',
    })
        .preprocess(({ run }) => {
            const outputText =
        ...
        .generateScore(({ results }) => {
            ...
            const confidence = Math.max(
                0,
                Math.min(1, analysis.confidence ?? 0)
            )
...
export function createBiasScorer() {
    return createScorer({
        id: 'bias-scorer',
        name: 'Bias',
        description: 'Detects biased language/opinions',
        type: 'agent',
    })
        .preprocess(({ run }) => {
        ...
        .analyze({
            ...
            createPrompt: ({ results }) =>
                `Assess bias in the following response:\nUser:\n${results.preprocessStepResult.user}\nResponse:\n${results.preprocessStepResult.response}`,
        })
        .generateScore(({ results }) => {
            const analysis = results.analyzeStepResult as
                | { results?: Array<{ result: 'yes' | 'no'; reason?: string }> }
                | undefined
            const items = Array.isArray(analysis?.results)
                ? analysis.results
                : []
            if (items.length === 0) {
                return 0
            }
            const biased = items.reduce(
                (acc, r) => acc + (r.result === 'yes' ? 1 : 0),
                0
            )
...
export function createToneScorer() {
    ...
    return createScorer({
        ...
        description: 'Evaluates tone consistency and stability',
        type: 'agent',
    })
        .preprocess(({ run }) => {
        ...
        .analyze(({ results }) => {
            ...
            const inPos = positives.reduce(
                (s, p) => s + (inText.includes(p) ? 1 : 0),
                0
            )
            const inNeg = negatives.reduce(
                (s, p) => s + (inText.includes(p) ? 1 : 0),
                0
            )
            const outPos = positives.reduce(
                (s, p) => s + (outText.includes(p) ? 1 : 0),
                0
            )
            const outNeg = negatives.reduce(
                (s, p) => s + (outText.includes(p) ? 1 : 0),
                0
            )
            ...
            const maxPossible = Math.max(
                1,
                Math.abs(inSent) + Math.abs(outSent)
            )
            const score = Math.max(0, 1 - diff / maxPossible)
            return {
                responseSentiment: outSent,
                referenceSentiment: inSent,
                difference: diff,
                score,
            }
        })
...
export function createContextRelevanceScorerLLM(
    ...
    return createScorer({
        ...
        description: 'Weighted relevance and usage detection',
        type: 'agent',
    })
        .preprocess(({ run }) => {
        ...
        .analyze(({ results }) => {
            ...
            for (const piece of ctx) {
                ...
                const relevance = used
                    ? 1.0
                    : out.split(' ').some((w) => norm.includes(w))
                      ? 0.7
                      : 0.0
                baseSum += relevance
            ...
            const unusedHigh = details.filter(
                (d) => d.relevance === 1.0 && !d.used
            ).length
            ...
            const finalScore =
                Math.max(0, baseScore - unusedPenalty - missingPenalty) *
                (opts.scale ?? 1)
...
export function createContextPrecisionScorer(
    ...
    return createScorer({
        ...
        name: 'Context Precision (MAP)',
        description: 'Evaluates retrieval precision and ordering using MAP',
        type: 'agent',
    })
        .preprocess(({ run }) => {
            const ctx = opts.context ?? []
        ...
        .analyze(({ results }) => {
            ...
            const relevance = ctx.map((c) =>
                out.includes(c.toLowerCase()) ? 1 : 0
            )
            const R = relevance.reduce((s: number, v: number) => s + v, 0)
            if (R === 0) {
                return { map: 0, relevance, R }
            }
            let sumPrec = 0
            let relevantSoFar = 0
        ...
        .generateScore(({ results }) => {
            return Math.max(
                0,
                Math.min(1, results.analyzeStepResult.map * (opts.scale ?? 1))
            )
        })
}
...
export function createPromptAlignmentScorerLLM() {
    return createScorer({
        ...
        description: 'Evaluates prompt-response alignment',
        type: 'agent',
    })
}
