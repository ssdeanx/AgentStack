define('vs/tsMode-CZz1Umrk', [
    ...
    function D(g, e, t = 0) {
        ...
        if (((o += g.messageText), t++, g.next))
            for (const n of g.next) o += D(n, e, t)
        return o
    }
    function S(g) {
        ...
        _convertRelatedInformation(e, t) {
            ...
            return (
                t.forEach((n) => {
                    ...
                    if (
                        (n.file &&
                            (l = this._libFiles.getOrCreateModel(
                                n.file.fileName
                            )),
                        !l)
                    )
    ...
    class C extends k {
                ...
                : {
                      suggestions: u.entries.map((m) => {
                          ...
                          if (m.replacementSpan) {
                              const x = e.getPositionAt(
                                      m.replacementSpan.start
                                  ),
                                  T = e.getPositionAt(
                                      m.replacementSpan.start +
                                          m.replacementSpan.length
                                  )
                              w = new i.Range(
                                  x.lineNumber,
                                  x.column,
                          ...
                          return (
                              m.kindModifiers !== void 0 &&
                                  m.kindModifiers.indexOf('deprecated') !==
                                      -1 &&
                                  A.push(
        ...
        async resolveCompletionItem(e, t) {
            const o = e,
                ...
                ).getCompletionEntryDetails(n.toString(), d, o.label)
            return r
                ? {
                      uri: n,
                      position: l,
...
${R(o)}`
    ...
    function R(g) {
        ...
        if (g.name === 'param' && g.text) {
            const [t, ...o] = g.text
            ;((e += `\`${t.text}\``),
                o.length > 0 && (e += ` — ${o.map((n) => n.text).join(' ')}`))
        } else
                        ...
                        u.parameters.forEach((m, w, A) => {
                            const x = S(m.displayParts),
                                T = {
                                    label: x,
                                    documentation: {
                                        value: S(m.documentation),
                                    },
                                }
                            ;((f.label += x),
                                f.parameters.push(T),
                                w < A.length - 1 &&
                                    (f.label += S(u.separatorDisplayParts)))
                        }),
    ...
    class H extends k {
        async provideDocumentHighlights(e, t, o) {
            const n = e.uri,
                ...
                d = await this._worker(n)
            if (e.isDisposed()) return
            const a = await d.getDocumentHighlights(n.toString(), l, [
                n.toString(),
            ])
            if (!(!a || e.isDisposed()))
                return a.flatMap((r) =>
        ...
        async provideDefinition(e, t, o) {
            ...
            if (
                ...
                (await this._libFiles.fetchLibFilesIfNecessary(
                    a.map((c) => i.Uri.parse(c.fileName))
                ),
                e.isDisposed())
            )
                return
        ...
        async provideReferences(e, t, o, n) {
            ...
            for (let u of r) {
                ...
                f &&
                    c.push({
                        uri: f.uri,
                        range: this._textSpanToRange(f, u.textSpan),
                    })
            }
            return c
        }
...
`,
    ...
    class V extends v {
        ...
        async provideDocumentRangeFormattingEdits(e, t, o, n) {
            ...
            if (e.isDisposed()) return
            const c = await r.getFormattingEditsForRange(
                l.toString(),
                d,
                a,
                v._convertOptions(o)
            ...
            if (!(!c || e.isDisposed()))
                return c.map((u) => this._convertTextChanges(e, u))
        }
    }
    class j extends v {
...
`,
        ...
        async provideOnTypeFormattingEdits(e, t, o, n, l) {
            ...
            if (e.isDisposed()) return
            const c = await r.getFormattingEditsAfterKeystroke(
                d.toString(),
                a,
                o,
                v._convertOptions(n)
            ...
            return !f || e.isDisposed()
                ...
                : {
                      actions: f
                          .filter(
                              (w) =>
                                  w.changes.filter((A) => A.isNewFile)
                                      .length === 0
                          )
                          .map((w) =>
                              this._tsCodeFixActionToMonacoCodeAction(e, o, w)
                          ),
                      dispose: () => {},
                  }
        }
            ...
            for (const d of o.changes)
                for (const a of d.textChanges)
                    n.push({
                        ...
                        versionId: void 0,
                        textEdit: {
                            range: this._textSpanToRange(e, a.span),
                            text: a.newText,
                        },
                    })
            return {
    ...
    class U extends k {
        ...
        async provideRenameEdits(e, t, o, n) {
            const l = e.uri,
                ...
                r = await this._worker(l)
            if (e.isDisposed()) return
            const c = await r.getRenameInfo(d, a, {
    ...
    function $(g, e) {
        ...
        function d() {
            ...
            ;(Z(t),
                a.completionItems &&
                    t.push(
                        i.languages.registerCompletionItemProvider(e, new C(n))
                    ),
                a.signatureHelp &&
                    t.push(
                        i.languages.registerSignatureHelpProvider(e, new F(n))
                    ),
                a.hovers &&
                    t.push(i.languages.registerHoverProvider(e, new N(n))),
                ...
                a.definitions &&
                    t.push(
                        i.languages.registerDefinitionProvider(e, new _(l, n))
                    ),
                a.references &&
                    t.push(
                        i.languages.registerReferenceProvider(e, new E(l, n))
                    ),
                a.documentSymbols &&
                    t.push(
                        i.languages.registerDocumentSymbolProvider(e, new W(n))
                    ),
                a.rename &&
                    t.push(i.languages.registerRenameProvider(e, new U(l, n))),
                ...
                a.codeActions &&
                    t.push(i.languages.registerCodeActionProvider(e, new K(n))),
                a.inlayHints &&
                    t.push(i.languages.registerInlayHintsProvider(e, new B(n))),
