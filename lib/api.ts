...
export async function listAgentsTyped(opts?: {
    ...
    | Record<string, Agent>
    | { agents: Record<string, Agent>; pagination?: unknown }
> {
    const base =
    ...
    try {
        ...
        try {
            ...
            throw new Error(
                'listAgents failed: ' + normalizeError(err ?? fallbackErr)
            )
        }
    }
}
...
export async function getMcpServerToolsTyped(
    serverId: string
    ...
    try {
        const res = await fetch(
            `${base.replace(/\/$/, '')}/api/mcp/servers/${encodeURIComponent(serverId)}/tools`,
            {
                credentials: 'include',
                headers: { Accept: 'application/json' },
            }
...
export async function agentGenerateTyped(
    agentId: string,
    ...
    try {
        const res = await fetch(
            `${base.replace(/\/$/, '')}/api/agents/${encodeURIComponent(agentId)}/generate`,
            {
                method: 'POST',
                credentials: 'include',
                headers: {
        ...
        try {
            ...
            throw new Error(
                'agentGenerate failed: ' + normalizeError(err ?? fallbackErr)
            )
        }
    }
}

export async function agentExecuteToolTyped(
    agentId: string,
    toolId: string,
    data: Record<string, unknown>,
    opts?: { threadId?: string; resourceId?: string; runId?: string }
) {
    const base =
        process.env.NEXT_PUBLIC_MASTRA_API_URL ?? 'http://localhost:4111'
    try {
        const res = await fetch(
            `${base.replace(/\/$/, '')}/api/agents/${encodeURIComponent(agentId)}/tools/${encodeURIComponent(toolId)}/execute`,
            {
                method: 'POST',
                credentials: 'include',
                headers: {
        ...
        try {
            ...
            throw new Error(
                'agentExecuteTool failed: ' + normalizeError(err ?? fallbackErr)
            )
        }
    }
}
...
// Memory wrappers
export async function createMemoryThreadTyped(params: {
    title: string
    resourceId: string
    agentId: string
    ...
    try {
        ...
        const base =
            process.env.NEXT_PUBLIC_MASTRA_API_URL ?? 'http://localhost:4111'
        const res = await fetch(
            `${base.replace(/\/$/, '')}/api/memory/threads`,
            {
                method: 'POST',
...
export async function deleteMemoryThreadTyped(threadId: string) {
    const base =
        process.env.NEXT_PUBLIC_MASTRA_API_URL ?? 'http://localhost:4111'
    const res = await fetch(
        `${base.replace(/\/$/, '')}/api/memory/threads/${encodeURIComponent(threadId)}`,
        {
            method: 'DELETE',
            credentials: 'include',
        }
...
export async function saveMessagesToMemoryTyped(
    messages: unknown[],
    agentId?: string
) {
    try {
        ...
        const base =
            process.env.NEXT_PUBLIC_MASTRA_API_URL ?? 'http://localhost:4111'
        const res = await fetch(
            `${base.replace(/\/$/, '')}/api/memory/threads/messages`,
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
...
export async function getWorkingMemoryTyped(
    agentId: string,
    threadId: string,
    resourceId?: string
) {
    try {
        ...
        const res = await fetch(
            `${base.replace(/\/$/, '')}/api/memory/working${qs}`,
...
export async function updateWorkingMemoryTyped(params: {
    agentId: string
    threadId: string
    ...
    try {
        ...
        const base =
            process.env.NEXT_PUBLIC_MASTRA_API_URL ?? 'http://localhost:4111'
        const res = await fetch(
            `${base.replace(/\/$/, '')}/api/memory/working`,
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
...
export async function listToolsTyped(opts?: {
    page?: number
    perPage?: number
}): Promise<
    Record<string, Tool> | { tools: Record<string, Tool>; pagination?: unknown }
> {
    const base =
        process.env.NEXT_PUBLIC_MASTRA_API_URL ?? 'http://localhost:4111'
    try {
        ...
        try {
            ...
            throw new Error(
                'listTools failed: ' + normalizeError(err ?? fallbackErr)
            )
        }
    }
}
...
export async function listWorkflowsTyped(opts?: {
    page?: number
    perPage?: number
}): Promise<
    | Record<string, Workflow>
    | { workflows: Record<string, Workflow>; pagination?: unknown }
> {
    const base =
        process.env.NEXT_PUBLIC_MASTRA_API_URL ?? 'http://localhost:4111'
    try {
        ...
        if (
            ...
            typeof data === 'object' &&
            data !== null &&
            Object.prototype.hasOwnProperty.call(data, 'workflows')
        ) {
            return data as {
        ...
        try {
            ...
            throw new Error(
                'listWorkflows failed: ' + normalizeError(err ?? fallbackErr)
            )
        }
    }
}

export async function getTracesTyped(params?: {
    page?: number
    perPage?: number
    ...
    try {
        ...
        try {
            ...
            const res = await fetch(
                `${base.replace(/\/$/, '')}/api/telemetry/traces${qs}`,
                {
                    credentials: 'include',
                    headers: { Accept: 'application/json' },
                }
            ...
            throw new Error(
                'getTraces failed: ' + normalizeError(err ?? fallbackErr)
            )
        }
    }
}

export async function listMemoryThreadsTyped(
    resourceId?: string,
    agentId?: string,
    ...
    try {
        ...
        const base =
            process.env.NEXT_PUBLIC_MASTRA_API_URL ?? 'http://localhost:4111'
        const res = await fetch(
            `${base.replace(/\/$/, '')}/api/memory/threads${qs}`,
            { credentials: 'include', headers: { Accept: 'application/json' } }
        )
        if (!res.ok) {
            throw new Error(`Status ${res.status}`)
        ...
        if (Array.isArray(data.threads)) {
            return data.threads as MemoryThread[]
        }
        // Fallback to client SDK
        const sdkRes = await mastraClient.listMemoryThreads({
...
export async function getThreadMessagesTyped(
    threadId: string,
    opts?: { page?: number; perPage?: number }
) {
    try {
        ...
        const base =
            process.env.NEXT_PUBLIC_MASTRA_API_URL ?? 'http://localhost:4111'
        const res = await fetch(
            `${base.replace(/\/$/, '')}/api/memory/threads/${encodeURIComponent(threadId)}/messages${qs}`,
            { credentials: 'include', headers: { Accept: 'application/json' } }
        )
        if (!res.ok) {
            throw new Error(`Status ${res.status}`)
        ...
        try {
            ...
            return await thread.listMessages({
                page: opts?.page ?? 1,
                perPage: opts?.perPage ?? 50,
            })
        } catch (fallbackErr) {
            throw new Error(
                'getThreadMessages failed: ' +
                    normalizeError(err ?? fallbackErr)
            )
        }
    }
}
...
export async function listLogsTypedPaged(params?: {
    transportId?: string
    page?: number
    perPage?: number
}) {
    ...
    try {
        ...
        const res = await fetch(`${base.replace(/\/$/, '')}/api/logs${qs}`, {
            credentials: 'include',
            headers: { Accept: 'application/json' },
        })
        if (!res.ok) {
...
export async function vectorQueryTyped(
    vectorName: string,
    params: {
        indexName: string
        queryVector: number[]
